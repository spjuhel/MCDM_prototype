...
# Function to convert a criteria scale to weights
def scale_to_weights(criteria_scale, scale_range=(1, 5)):
    """
    Convert a criteria importance scale to weights that sum to 1.

    Parameters:
        criteria_scale (dict): A dictionary where keys are criteria names and values are importance scores.
        scale_range (tuple): A tuple specifying the minimum and maximum of the scale (e.g., (1, 5)).

    Returns:
        dict: A dictionary with criteria names and their corresponding weights.

    Raises:
        ValueError: If importance scores are outside the specified scale range or if the total importance is zero.
    """
    min_scale, max_scale = scale_range
    
    # Validate the scale range
    if min_scale < 0:
        raise ValueError("Scale range cannot include negative values.")
    if min_scale >= max_scale:
        raise ValueError("The minimum of the scale range must be less than the maximum.")

    # Validate importance scores against the scale
    ...
    # Normalize importance values to derive weights
    weights = {crit: imp / total_importance for crit, imp in criteria_scale.items()}
